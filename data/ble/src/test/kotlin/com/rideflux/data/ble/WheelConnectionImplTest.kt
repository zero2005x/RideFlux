...
class WheelConnectionImplTest {
    ...
    fun `faults set is replaced wholesale on each TelemetryUpdate`() = runTest {
        ...
        transport.emit(byteArrayOf(2))
        runCurrent()
        assertTrue(conn.telemetry.value.faults?.isEmpty() == true)
    }

    // ---- helpers -------------------------------------------------------

    @Test
    fun `keep-alive loop survives a throwing codec`() = runTest {
        val transport = FakeBleTransport()
        val codec = FakeWheelCodec(keepAlivePeriodMillis = 25L)
        // First tick throws (codec bug); later ticks write normally.
        var ticks = 0
        codec.onKeepAlive = {
            ticks += 1
            if (ticks == 1) throw RuntimeException("codec bug")
            listOf(byteArrayOf(0x55))
        }
        val conn = connection(transport, codec, backgroundScope)
        ...
        advanceTimeBy(26L)  // first tick: keepAliveFrames throws — loop must survive
        runCurrent()
        advanceTimeBy(25L)  // second tick: writes 0x55
        runCurrent()
        advanceTimeBy(25L)  // third tick: writes 0x55
        runCurrent()
    ...
    @Test
    fun `keep-alive stops writing after the ingest loop reports link loss`() = runTest {
        val transport = FakeBleTransport()
        val codec = FakeWheelCodec(keepAlivePeriodMillis = 10L)
        codec.onKeepAlive = { listOf(byteArrayOf(0x55)) }
        val conn = connection(transport, codec, backgroundScope)
        ...
        // Simulate link loss detected by the ingest loop.
        transport.emitFailure(RuntimeException("gatt link dropped"))
        runCurrent()

        val st = conn.state.value
        assertTrue("state was $st", st is ConnectionState.Failed)

        // Timer ticks after the failure must produce no writes.
        advanceTimeBy(100L)
        runCurrent()
        assertTrue("writes after link loss: ${transport.writes.size}", transport.writes.isEmpty())
