...
object NinebotCommandBuilder {
    ...
    fun build(
        ...
        // Derived from the frame itself (NinebotFrame.lenByte) so the
        // two length conventions cannot silently drift apart.
        val lenByte = frame.lenByte
     ...
     *
     * [gamma] is REQUIRED (no zero-keystream default): N2 data
     * commands must be encrypted with the session key, so forgetting
     * to pass it should be a compile-time error rather than silently
     * sending plaintext. Use the key captured from the GetKey reply.
     */
    fun readParam(
        param: Int,
        readLength: Int = 1,
     ...
     * [gamma] is REQUIRED — see [readParam].
     */
    fun writeParam(
        param: Int,
        payload: ByteArray,
