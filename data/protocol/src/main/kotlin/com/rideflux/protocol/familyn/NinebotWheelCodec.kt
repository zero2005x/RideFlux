...
abstract class NinebotWheelCodecBase internal constructor(
        ...
        while (progress && s.buffer.size >= 2) {
            ...
            when (val r = NinebotDecoder.decode(wire, wireFamily, s.gamma, offset = 0)) {
                is NinebotDecodeResult.Ok -> {
                    ...
                    // Guard against a zero-consumption Ok (would otherwise
                    // spin the loop forever without draining any input).
                    progress = r.consumedBytes > 0
                    ...
                    if (r.frame.param == 0xB0 && r.frame.data.size >= NinebotTelemetryB0.MIN_DATA_SIZE) {
                        // Identification is emitted only after a real
                        // telemetry frame has been decoded AND, for N2,
                        // the GetKey keystream has been captured — marking
                        // identified earlier would leave gamma at zero and
                        // the session unrecoverable.
                        val n2Ready = wireFamily != NinebotFamily.N2 ||
                            s.gamma.any { it.toInt() != 0 }
                        if (!s.identified && n2Ready) {
                            s.identified = true
                            events.add(
                        ...
                        val t = runCatching { NinebotTelemetryB0.parse(r.frame.data) }.getOrNull()
                        if (t == null) {
                            events.add(
                                DecodeEvent.Malformed(
                                    reason = "Ninebot ${wireFamily}: B0 page too short",
                                    offendingBytes = r.frame.data,
                                ),
                            )
                        } else {
                ...
                is NinebotDecodeResult.Fail -> {
                    when (r.error) {
                        is NinebotDecodeError.TooShort -> {
                            ...
                            // Resync by scanning forward to the next
                            // two-byte frame prefix (55 AA) and emit ONE
                            // Malformed event per resync, instead of
                            // dropping one byte at a time (which would
                            // flood consumers with near-identical events
                            // on a garbage burst).
                            val prefixIdx = (0 until s.buffer.size - 1).indexOfFirst {
                                s.buffer[it].toInt() == 0x55 && s.buffer[it + 1].toInt() == 0xAA
                            }
                            val dropCount = if (prefixIdx > 0) prefixIdx else 1
                            repeat(dropCount) { s.buffer.removeAt(0) }
                            events.add(
                                DecodeEvent.Malformed(
    ...
    override fun encode(state: WheelCodec.State, command: WheelCommand): List<ByteArray> =
        when (command) {
            is WheelCommand.Raw -> {
                // decode() decrypts every inbound N2 frame with the
                // session γ — outbound commands must be encrypted with
                // the same keystream or the wheel rejects them
                // (asymmetric encode/decode). N1 frames always use the
                // identity keystream, so applying γ is a no-op there.
                val s = state as NinebotState
                val obscured = command.bytes.copyOf()
                NinebotCodec.xorInPlace(obscured, s.gamma)
                listOf(obscured)
            }
            else -> emptyList()
        }
