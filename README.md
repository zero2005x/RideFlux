...
| 魔術位元組 | `0x52`（`'R'`） |
| 協定版本 | `2`（20 位元組封包） |
| 封包大小（v2） | 20 位元組，小端序——可容納預設的 23 位元組 ATT MTU |
| 偏好 ATT MTU | 64（由用戶端發起協商；v2 完全不依賴它） |
| Rokid CXR 通道 | `rideflux.telemetry.v1` |

封包內容：時間戳（以秒為單位，解碼為無號數）、時速、車輛電量 %、手機電量 %、電池組電壓、
行程距離、行程時間、粗略訊號等級、資料過期旗標、就緒旗標。每個數值欄位皆以哨兵值表示
「無資料」。v2 版面把訊號等級併入旗標位元組，並將時間戳縮為一個 word，因此單一通知必定
承載一個完整封包；先前 32 位元組的 v1 版面僅保留解碼路徑，讓眼鏡端 APK 比手機先更新時，
在混合安裝期間仍讀得到封包。任何破壞性變更都必須遞增 `PROTOCOL_VERSION`。
