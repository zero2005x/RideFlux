...
object KingSongCommandBuilder {
    ...
    fun setAlarmAndMaxSpeed(
        ...
        // Degenerate-case rewrite (all zeros → 0x98 query) is only
        // applied when the caller did not explicitly request a write.
        val allZero = alarm1Kmh == 0 && alarm2Kmh == 0 && alarm3Kmh == 0 && maxSpeedKmh == 0
        val code = if (allZero && !forceWrite) {
    ...
    fun setLightMode(lightMode: Int): ByteArray {
        require((0x12 + lightMode) in 0..0xFF) { "lightMode out of range (would overflow byte 2)" }
        val payload = ByteArray(14)
