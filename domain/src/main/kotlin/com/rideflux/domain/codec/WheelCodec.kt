...
interface WheelCodec {
     ...
     *
     * NOT thread-safe: the caller must serialize all decode/encode/
     * keepAliveFrames calls on the same [State] (e.g. a single-threaded
     * dispatcher or a Mutex); concurrent calls race on the internal
     * reassembly buffers and can corrupt parsing state.
     */
    interface State
     ...
     *
     * Must NOT throw for malformed input; protocol errors must be
     * reported as [DecodeEvent.Malformed] so the connection can keep
     * running.
     *
     * Callers must invoke [decodeSafely] rather than this method
     * directly: it converts any escaping exception into a
     * [DecodeEvent.Malformed] so that a single bad frame can never tear
     * down the GATT session.
     */
    fun decode(state: State, bytes: ByteArray): List<DecodeEvent>

    /**
     * [decode] with the "must not throw" guarantee made structural
     * rather than documentary.
     *
     * Implementations are still expected to be fail-soft, but wire data
     * is untrusted and a missed bounds check must degrade to a
     * [DecodeEvent.Malformed] event, not an exception on the BLE
     * callback path. [CancellationException] is not caught here because
     * [decode] is non-suspending and must never observe one.
     */
    fun decodeSafely(state: State, bytes: ByteArray): List<DecodeEvent> =
        try {
            decode(state, bytes)
        } catch (e: Exception) {
            listOf(
                DecodeEvent.Malformed(
                    reason = "codec threw ${e.javaClass.simpleName}: ${e.message}",
                    offendingBytes = bytes,
                ),
            )
        }
    ...
    /**
     * Desired keep-alive period in milliseconds; ignored if
     * [keepAliveFrames] is empty.
     *
     * A non-empty frame list REQUIRES a strictly positive period —
     * `0L` here means "no keep-alive", never "send as fast as
     * possible". Use [keepAliveSpec] rather than reading this pair
     * directly; it validates the combination so a codec that overrides
     * only one of the two cannot make the connection layer busy-loop.
     */
    val keepAlivePeriodMillis: Long get() = 0L

    /**
     * Validated keep-alive configuration, or `null` when this family
     * needs no keep-alive.
     *
     * Connection layers must use this instead of pairing
     * [keepAliveFrames] with [keepAlivePeriodMillis] themselves.
     */
    fun keepAliveSpec(state: State): KeepAliveSpec? {
        val frames = keepAliveFrames(state)
        if (frames.isEmpty()) return null
        val period = keepAlivePeriodMillis
        require(period > 0L) {
            "${this::class.java.simpleName} returns ${frames.size} keep-alive " +
                "frame(s) but keepAlivePeriodMillis is $period; a non-empty " +
                "frame list requires a positive period"
        }
        return KeepAliveSpec(frames = frames, periodMillis = period)
    }

    /** A non-empty keep-alive frame list paired with a positive cadence. */
    data class KeepAliveSpec(
        val frames: List<ByteArray>,
        val periodMillis: Long,
    ) {
        init {
            require(frames.isNotEmpty()) { "frames must not be empty" }
            require(periodMillis > 0L) { "periodMillis must be > 0" }
        }
    }
}
