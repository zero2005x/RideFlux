 ...
 * of two enumerations, selected by model family (see the model-id
 * tables in §3.5.2 — the spec lists some models in both tables, e.g.
 * V10 / V10S / V10SF, so callers should pick the convention that
 * matches the actual model):
 *
 * - [Convention.LEGACY] masks `state & 0x0F`; the legacy list also
 *   includes V5F / V5+ / V5D / Glide3 / V10.
 * - [Convention.MODERN] reads the *low-byte* high nibble
 *   `(state >>> 4) & 0x0F` and appends `" - Engine off"` whenever
 *   `state & 0x0F == 1`; the modern list also includes V10S / V10SF.
 *
 * The caller is responsible for picking the right convention from the
 * model-id tables in §3.5.2.
 */
...
data class InmotionI1AlertRecord(
     ...
     * per-model value from §3.5.2 — see [aSpeedKmh].
     */
    val aSpeedKmh: Double get() = aSpeedKmh(f = 3812.0)

    /**
     * Same as [aSpeedKmh] but with an explicit per-model calibration
     * factor [f] so models whose alert-path calibration differs (e.g.
     * R1T uses 3810) can be decoded correctly instead of silently
     * producing wrong km/h.
     */
    fun aSpeedKmh(f: Double): Double =
        kotlin.math.abs(aValue2 / f) * 3.6
