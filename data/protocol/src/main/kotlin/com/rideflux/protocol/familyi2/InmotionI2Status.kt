 ...
 * and `"Lifted"` (state-B bit 2) in that order, joined by single
 * spaces. Absent flags contribute nothing. Returns the empty string
 * when no flag is set.
 */
fun inmotionI2StatusString(a: InmotionI2StateA, b: InmotionI2StateB): String {
    val tokens = mutableListOf<String>()
    if (a.motorActive) tokens += "Active"
    if (a.charging) tokens += "Charging"
    if (b.lifted) tokens += "Lifted"
...
data class InmotionI2ErrorBitmap(
    ...
    /** `true` iff any *known* error bit in `E0..E6` is set. Reserved /
     * never-read bits are masked out so a frame carrying only reserved
     * bits does not falsely report a fault. */
    val hasAnyFault: Boolean
        get() = (e0 or e1 or
            (e2 and 0x07) or
            (e3 and 0xC3) or e4 or e5 or
            (e6 and 0x06)) != 0 ||
            overBusCurrentSeverity >= InmotionI2Severity.WARNING ||
            lowBatterySeverity >= InmotionI2Severity.WARNING
