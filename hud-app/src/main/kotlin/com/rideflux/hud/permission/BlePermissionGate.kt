...
fun BlePermissionGate(content: @Composable () -> Unit) {
    ...
    // Re-evaluate on every resume so permissions granted (or revoked)
    // in system Settings are picked up without a recreation.
    ...
    // Auto-prompt at most once per session so a rotation / recomposition
    // after a denial does not immediately re-trigger the system dialog
    // (which on Android 11+ can consume the "second denial" quota that
    // leads to permanent denial without user intent). Retries happen via
    // the on-screen button.
    var autoPrompted by rememberSaveable { mutableStateOf(false) }
    LaunchedEffect(Unit) {
        if (!granted && !autoPrompted) {
            autoPrompted = true
    ...
    if (granted) {
        ...
        Column(
            ...
            Button(
                onClick = {
                    // Once the system stops showing the dialog (permanent
                    // denial), re-launching is a no-op — open the app
                    // settings page so the user can grant manually.
                    val intent = Intent(
