...
data class WheelTelemetry(
    ...
    // ---- Battery --------------------------------------------------------
    /**
     * Best-estimate battery state of charge, 0..100. Invariant enforced
     * at the domain boundary: codecs/sources must not emit out-of-range
     * or non-finite values (0..100 and finite), or UI/logic will
     * propagate them unchecked.
     */
    ...
    /**
     * Set of faults reported by the latest frame, or `null` when no
     * fault-reporting frame has been decoded yet. `null` means
     * "unknown", NOT "no faults", so consumers (UI, safety logic) can
     * distinguish a wheel that has not reported from a confirmed
     * healthy wheel.
     */
    ...
    init {
        ...
        // here so it holds for every construction path, including
        // `copy()`. Codec decode loops convert the resulting
        // IllegalArgumentException into a Malformed event, so a bad frame
        // degrades instead of propagating NaN into UI/safety logic.
        require(
            batteryPercent == null ||
                (batteryPercent.isFinite() && batteryPercent in 0f..100f),
        ) { "batteryPercent must be finite and within 0..100, got $batteryPercent" }
    }

    companion object {
        /**
         * Empty snapshot used as the `initialValue` of the telemetry
         * StateFlow. Note [timestampMillis] uses `0L` as the "no data"
         * sentinel; genuine readings taken immediately after a monotonic
         * clock starts (e.g. right after boot) could theoretically be 0,
         * so consumers should treat `timestampMillis <= 0L` as "no frame".
         */
