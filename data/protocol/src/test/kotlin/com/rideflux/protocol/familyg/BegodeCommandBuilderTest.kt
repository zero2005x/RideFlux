...
class BegodeCommandBuilderTest {
    ...
    @Test fun `wheel calibration is two ASCII bytes c y with a delay`() {
        val cal = BegodeCommandBuilder.wheelCalibration()
        assertEquals(1, cal.first.size)
        assertEquals('c'.code.toByte(), cal.first[0])
        assertEquals(300L, cal.delayMs)
        assertEquals(1, cal.second.size)
        assertEquals('y'.code.toByte(), cal.second[0])
    }
}
