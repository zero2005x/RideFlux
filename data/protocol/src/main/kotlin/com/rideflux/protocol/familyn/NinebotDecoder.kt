 ...
 * only the presence of a CMD byte distinguishes them. The family
 * MUST be known a priori (connection state / handshake); there is no
 * checksum-based way to disambiguate, because §6.1 covers the exact
 * same physical bytes under both interpretations (a valid N2 frame
 * also decodes successfully as N1). Callers that guess the family
 * can silently mis-parse, so the family must come from the domain
 * layer, not from a decoder heuristic.
 */
object NinebotDecoder {
    ...
    fun decode(
        ...
        // Reject negative offsets: `wire.size - offset` is *larger*
        // than wire.size for offset < 0, so both bounds checks would
        // pass and wire[offset] / System.arraycopy would throw
        // ArrayIndexOutOfBoundsException instead of returning Fail.
        if (offset < 0 || wire.size - offset < fixedHeader) {
