...
class InmotionI1EncoderDecoderTest {
    ...
    @Test fun buildStandard_producesRoundTrippableFrame() {
        ...
        // FMT = 0x01 (extended): every CAN_ID_* here is a 29-bit
        // extended identifier, which exceeds the 11-bit standard range.
        assertEquals(0x01, ok.frame.fmt)
    ...
    fun buildStandard_rejectsCanIdsWiderThan29Bits() {
        InmotionI1CommandBuilder.buildStandard(0x20000000L, ByteArray(8))
    }

    @Test(expected = IllegalArgumentException::class)
    fun setPedalSensitivity_rejectsOverflowingValues() {
        InmotionI1CommandBuilder.setPedalSensitivity(Int.MAX_VALUE)
    }
    ...
    @Test fun decode_negativeOffset_returnsFailNotCrash() {
        val wire = InmotionI1CommandBuilder.requestLiveTelemetry()
        val result = InmotionI1Decoder.decode(wire, offset = -1)
    ...
    @Test fun decode_parsesExtendedFrameWithExData() {
        ...
        wire[2 + escaped.size] = check.toByte()
