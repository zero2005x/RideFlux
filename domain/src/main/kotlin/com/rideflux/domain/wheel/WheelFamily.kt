 ...
 *
 * **Stability contract:** the enum *name* is used directly as the
 * routing / persistence key (persisted family strings, nav deep
 * links, [WheelCodecFactory] routing). Renaming or reordering an
 * entry is a wire/persistence-breaking change: previously stored
 * family strings would fail to parse and silently fall back to
 * [WheelFamily.G]. A unit test pins the mapping between the spec's
 * family-key list and [entries] so this is verified, not just
 * documented.
 *
