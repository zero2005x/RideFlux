...
internal object InmotionI1Codec {
     ...
     * body.
     *
     * A wire `A5` acts as an escape marker: per §2.6.2 it must be
     * followed by one of the three reserved bytes (`0xAA`/`0x55`/
     * `0xA5`). Any other byte after `A5` is malformed and rejected via
     * return value `null` (the weak additive checksum alone is not a
     * reliable corruption signal). A lone trailing `A5` also returns
     * `null`.
     */
    fun unescape(wire: ByteArray, offset: Int, escapedLength: Int): ByteArray? {
        ...
        val out = ByteArray(escapedLength)
        var n = 0
        var i = offset
        while (i < end) {
            val b = wire[i]
            if (b == ESCAPE_BYTE) {
                ...
                val e = wire[i + 1]
                if (!mustEscape(e.toInt() and 0xFF)) return null
                out[n++] = e
                i += 2
            } else {
                out[n++] = b
     ...
     * `CHECK = (Σ body[i]) mod 256`. Returned as an unsigned `Int` in
     * `0..255` so callers can compare against wire bytes without an
     * `and 0xFF` dance.
     */
    fun checksum(body: ByteArray): Int = checksum8(body)
