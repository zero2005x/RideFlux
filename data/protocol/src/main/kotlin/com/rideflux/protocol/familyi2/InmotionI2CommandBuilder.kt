...
object InmotionI2CommandBuilder {
    ...
    fun build(flags: Int, cmd: Int, data: ByteArray): ByteArray {
        ...
        require(cmd in 0..0x7F) {
            "cmd out of range (reserved high bit must be clear): $cmd"
        }
    ...
    fun setMaxSpeed(maxKmh: Double): ByteArray {
        require(maxKmh.isFinite() && maxKmh in 0.0..655.35) {
            "max speed out of range: $maxKmh"
        }
        return controlU16BE(0x21, (maxKmh * 100.0).toInt())
    }
    ...
    fun setMaxSpeedWithAlarm1(maxKmh: Double, alarmKmh: Double): ByteArray {
        ...
        require(alarmKmh.isFinite() && alarmKmh in 0.0..655.35) {
            "alarm speed out of range: $alarmKmh"
        }
        val maxRaw = (maxKmh * 100.0).toInt()
        val alarmRaw = (alarmKmh * 100.0).toInt()
        return control(
            0x21,
            byteArrayOf(
    ...
    fun setPedalHorizonTilt(angleDegrees: Double): ByteArray {
        require(angleDegrees.isFinite() && angleDegrees in 0.0..6553.5) {
            "pedal horizon angle out of range: $angleDegrees"
        }
        return controlU16BE(0x22, (angleDegrees * 10.0).toInt())
    }
    ...
    fun setStandbyDelay(delayMinutes: Int): ByteArray =
        controlU16BE(
            0x28,
            (delayMinutes.toLong() * 60L).coerceIn(0L, 0xFFFFL).toInt(),
        )
