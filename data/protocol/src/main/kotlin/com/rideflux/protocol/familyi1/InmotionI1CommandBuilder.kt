...
object InmotionI1CommandBuilder {
    ...
    fun buildStandard(
        ...
        require(canId in 0..0x1FFFFFFFL) {
            "canId must be a 29-bit extended identifier: $canId"
        }
        ...
        // Every CAN_ID_* in this file is a 29-bit extended identifier
        // (§3.5.1), which exceeds the 11-bit standard range — so the
        // envelope MUST carry FMT = 0x01 (extended), not 0x00.
        body[14] = FMT_EXTENDED.toByte()
        body[15] = type.toByte()
        return wrap(body)
    }
    ...
    fun setMaxSpeed(maxKmh: Double): ByteArray {
        require(maxKmh.isFinite()) { "max speed must be finite: $maxKmh" }
        require(maxKmh in 0.0..65.535) { "max speed out of range: $maxKmh" }
        val raw = (maxKmh * 1000.0).roundToInt()
    ...
    fun setPedalSensitivity(sens: Int): ByteArray {
        val rawLong = (sens.toLong() + 28L) shl 5
        require(rawLong in 0L..0xFFFFL) { "pedal sensitivity out of range: $sens" }
        val raw = rawLong.toInt()
        val data8 = ByteArray(8)
        data8[0] = 0x06
        data8[3] = ((raw ushr 8) and 0xFF).toByte()
        data8[4] = (raw and 0xFF).toByte()
    ...
    fun setHorizontalTilt(angleDegrees: Double): ByteArray {
        require(angleDegrees.isFinite()) { "tilt angle must be finite: $angleDegrees" }
        val raw = (angleDegrees * 6553.6).roundToLong()
        require(raw in 0..0xFFFFFFFFL) { "tilt angle out of range: $angleDegrees" }
        val data8 = ByteArray(8)
        data8[0] = 0x00
    ...
    private fun wrap(body: ByteArray): ByteArray {
        ...
        out[2 + escaped.size] = check.toByte()
